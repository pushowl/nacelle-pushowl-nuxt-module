...
const handleProductView = (product, variant) => {
  ...
  if (!variant.availableForSale) {
    window.pushowl.trigger('showWidget', {
      ...
      variant: {
        id: parseInt(decodeBase64Id(currentVariant.id), 10),
        title: variant.title,
...
const setFlyoutWidget = (show) => {
  if (typeof window !== 'undefined') {
    const flyoutWidget = document.querySelector('.pushowl-widget-node')
    if (flyoutWidget) {
      flyoutWidget.style.display = show ? 'block' : 'none'
      flyoutWidget.classList.remove('is-maximized')
    }
  }
}
...
export default function (ctx) {
  ...
  ctx.store.subscribe((mutation, state) => {
    ...
    const subscriberToken = localStorage.getItem('pushowl_subscriber_token')
    document.cookie = `pushowl_subscriber_token=${subscriberToken}`
    ...
    if (
      ...
      handleProductView(payload.product, variant)
