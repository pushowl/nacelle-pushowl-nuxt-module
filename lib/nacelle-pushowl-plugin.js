...
const cartToPushOwlObject = (cart) => {
  ...
  const checkoutToken = '' //atob(checkoutResponse.id).match(/Checkout\/(\S+)\?/)[1]
...
export default function (ctx) {
  ...
  if (
    ...
    window.location.search.match(/showCart/)
  ) {
  ...
  ctx.store.subscribe((mutation, state) => {
    ...
    if (type === 'checkout/setCheckout') {
      const checkoutId = atob(payload.id).match(/Checkout\/(\S+)\?/)[1]
